...
loop:
	for {
		...
		if isEINTR(err) {
			...
			log.Printf("zmq.Poll was interrupted")
			continue
		}
		if err != nil {
		...
		for _, p := range polled {
			...
			case s.resultPull:
				...
				if err == errLoopEnd {
					log.Printf("Exiting polling loop for %s", s.name)
					break loop
				}
				if err != nil {
					log.Printf("Failed to handle a message on the result socket of %s: %v", s.name, err)
				}
			default:
